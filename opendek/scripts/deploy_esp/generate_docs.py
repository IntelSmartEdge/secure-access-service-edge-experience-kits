...
def create_html_from_md(cfg):
    ...
    for path, file_name in md_files:
        with open(os.path.join(out_path, path, file_name), 'r', encoding="utf-8") as f:
            md_content = f.read()
            ...
            if "split" in path:
                with open(os.path.join(out_path, "html", path, file_name[:-3] + ".html"), 'w', encoding="utf-8") as f:
...
def get_md_sections(cfg, file_path, file_name):
    ...
    with open(os.path.join(out_path, "input", file_path, file_name), 'r', encoding="utf-8") as md_file:
        for line in md_file:
...
def write_md_sections(cfg, sections, file_path, file_name):
    ...
    for tag, lines in sections:
        with open(os.path.join(out_path, "split", file_path, file_name[:-3] + "_" + tag + ".md"), "w",
                               encoding="utf-8") as f:
            f.writelines(lines)
